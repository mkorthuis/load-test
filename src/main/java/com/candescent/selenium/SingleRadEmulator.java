...
public class SingleRadEmulator {
	...
	public void run() throws InterruptedException {
		//Wait a few seconds (randomly) so everything doesn't start at the same time.
		...
		//Runs in an infinite loop.  Will get killed by the master process when time is up.
		while(true) {
	 ...
	 */
	public void runEncounterPage(HtmlUnitDriver driver, Integer waitTimeMs, Integer studyId) throws InterruptedException {
		...
		driver.get(baseUrl + "/case/rad_auto_dictate.ajax.php?action=GET_NEXT&current_cases_id=2336814&worklist=personal&adqUserPref=off&type=");
		driver.get(baseUrl + "/api/v1/encounter/" + studyId);
		...
		executor.scheduleAtFixedRate(new Runnable() {
		    @Override 
		    public void run() {
				driver.get(baseUrl + "/case/rad_auto_dictate.ajax.php?action=GET_NEXT&current_cases_id=" + studyId + "&worklist=personal&adqUserPref=off&type=");
	 ...
	 */
	public void runWorklistPage(HtmlUnitDriver driver, Integer waitTimeMs, Integer studyId) throws InterruptedException {
		String baseUrl = arguments.getEnvironment().getUrl();
